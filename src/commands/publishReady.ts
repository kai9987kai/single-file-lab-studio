...
export async function publishReadyCommand(): Promise<void> {
    ...
    const parts = [];
    if (injectFavicon) { parts.push('favicon'); }
    if (injectSEO) { parts.push('SEO meta'); }
    if (injectSW) { parts.push('service worker'); }
